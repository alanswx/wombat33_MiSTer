| payload_entry_amiga.s — entry shim for the Amiga 68040 bench
| payloads (wombat33 port of the MacIIvi 68030 original). Entered by
| bootblock.s at $80000 with A1 = trackdisk IOStdReq, A6 = ExecBase.
|
| Responsibilities (Amiga analog of payload_entry_cpu.s on the Mac):
|   0. Zero .bss (finding 15: NOLOAD statics inherit RAM garbage).
...
|   4. 68040 gate (amiga_gate.c) -- halts with a painted message
|      on anything else.
|   5. bench_main() (shared runner).
|
...
_payload_start:
    | --- 0. zero .bss (before anything touches statics; finding 15) ---
    lea     _payload_bss_start, %a0
    lea     _payload_bss_end, %a2
1:  cmp.l   %a2, %a0
    bcc.s   2f
    clr.l   (%a0)+
    bra.s   1b
