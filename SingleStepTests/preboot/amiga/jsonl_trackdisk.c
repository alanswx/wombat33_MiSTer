...
/* 68040: push any dirty data-cache lines before handing a buffer to
 * trackdisk. On a bare boot exec leaves the 040 D-cache off (no
 * 68040.library), so this is insurance, not a dependency; CPUSHA is
 * legal on every 040 variant in supervisor mode and we are still in
 * supervisor when it runs. */
static void push_dcache(void)
{
    asm volatile (".short 0xF478 \n nop" : : : "memory");   /* CPUSHA DC */
}
