#!/usr/bin/env python3
"""Push (optional) and launch a MiSTer core.
...
TWO LAUNCH METHODS (--launch):
  cmd  (default when --ssh-key is set)  write `load_core <path>` to the MiSTer's
       /dev/MiSTer_cmd FIFO over ssh. Deterministic: no reboot, no menu geometry,
       no keystroke timing. This is the one to use.
  osd  drive the main-menu OSD blind with generated keystrokes (below). Kept for
       hosts with no ssh access. It DOES occasionally miss: on .143 (2026-08-29)
       it lost one keypress at the root menu on both tries, opening _Other instead
       of _Unstable and launching an unrelated core.
...
# -------------------------------------------------------------------------- seed
def launch_via_cmd(host, user, key, remote_core_path):
    """Launch by writing `load_core <path>` to the MiSTer's /dev/MiSTer_cmd FIFO.

    Deterministic, unlike the OSD path: no reboot, no menu geometry, no timing.
    Needs ssh (so --ssh-key), which --push already requires anyway. Verified on
    Main_MiSTer "Version 260828": `load_core ` is the only command the FIFO takes,
    and it accepts an .rbf or an .mgl.
    """
    print(f"[cmd] load_core {remote_core_path} -> /dev/MiSTer_cmd")
    subprocess.run(
        ["ssh", *_ssh_opts(key), f"{user}@{host}",
         f"echo {shlex.quote('load_core ' + remote_core_path)} > /dev/MiSTer_cmd"],
        check=True)
...
def verify(host, port, expect, tag="osd", settle=7, deadline=30):
    ...
    Polls until `deadline`: a big core takes ~15 s to configure the FPGA, and a
    single read at `settle` reported the OUTGOING core on the cmd path."""
    print(f"[{tag}] launch sent; waiting for the core to come up...")
    time.sleep(settle)
    running = None
    end = time.time() + max(0, deadline - settle)
    while True:
        try:
            running = asyncio.run(read_running(host, port))
        except Exception as e:
            print(f"[{tag}] WARN: could not reconnect to verify launch: {e}")
            return True
        if running and _core_matches(expect, running):
            break
        if time.time() >= end:
            break
        time.sleep(2)
    if running is None:
        print(f"[{tag}] WARN: no coreRunning broadcast seen - could not verify launch")
        return True
    if running == "":
        print(f"[{tag}] MISS: coreRunning empty - still at the menu (selection missed)")
        return False
    if _core_matches(expect, running):
        print(f"[{tag}] OK: coreRunning='{running}' - {expect} launched")
        return True
    print(f"[{tag}] MISS: launched coreRunning='{running}', expected '{expect}'")
    return False


def _core_matches(expect, running):
    ...
    return exp == r or (hi.startswith(lo) and not hi[len(lo)].isalnum())
...
def main():
    ...
    ap.add_argument("--launch", choices=("cmd", "osd"), default=None,
                    help="how to start the core: 'cmd' writes load_core to the "
                         "MiSTer's /dev/MiSTer_cmd FIFO over ssh (deterministic, no "
                         "reboot); 'osd' drives the main-menu OSD blind. Default: "
                         "cmd when an ssh key is available, else osd.")
    ...
    # Blind OSD navigation is timing-sensitive and DOES miss: on .143 (2026-08-29)
    # it lost one keypress at the root menu on both tries, opening _Other instead of
    # _Unstable and launching whatever row 68 landed on. The FIFO has no such failure
    # mode, so prefer it whenever ssh is configured.
    if args.launch is None:
        args.launch = "cmd" if args.ssh_key else "osd"
    ...
    # 2. launch. The FIFO path is deterministic; the OSD path is the fallback for
    # hosts with no ssh access.
    if args.launch == "cmd":
        remote_core = posixpath.join(folder_path, args.core)
        if args.dry_run:
            print(f"[cmd] (dry-run) would send: load_core {remote_core}")
            return 0
        launch_via_cmd(args.host, args.ssh_user, args.ssh_key, remote_core)
        if args.no_verify:
            return 0
        return 0 if verify(args.host, args.port, os.path.splitext(args.core)[0],
                           tag="cmd", settle=10) else 1
